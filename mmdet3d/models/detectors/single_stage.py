...
@DETECTORS.register_module()
class lg3d_SingleStage3DDetector(Base3DDetector):
    """SingleStage3DDetector.
    ...
    def __init__(self,
                 ...
                 init_cfg=None,
                 pretrained=None):
        super(lg3d_SingleStage3DDetector, self).__init__(init_cfg)
        self.s_backbone = build_backbone(s_backbone)
    ...
    def forward_dummy(self, points, label_points):
        """Used for computing network flops.

        See `mmdetection/tools/analysis_tools/get_flops.py`
        """
        x, lg_x = self.extract_feat(points, label_points)
        try:
            sample_mod = self.train_cfg.sample_mod
            outs = self.bbox_head(x, sample_mod)
