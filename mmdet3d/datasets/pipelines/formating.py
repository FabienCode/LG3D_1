...
@PIPELINES.register_module()
class lg3d_DefaultFormatBundle3D(DefaultFormatBundle):
    """Default formatting bundle.
    ...
    def __init__(self, class_names, with_gt=True, with_label=True):
        super(lg3d_DefaultFormatBundle3D, self).__init__()
        self.class_names = class_names
        self.with_gt = with_gt
        self.with_label = with_label
    ...
    def __call__(self, results):
        ...
        if 'label_points' in results:
            assert isinstance(results['label_points'], BasePoints)
            results['label_points'] = DC(results['label_points'].tensor)
                ...
                if 'gt_names_3d' in results:
                    results['gt_labels_3d'] = np.array([
                        ...
                        for n in results['gt_names_3d']
                                                       ...
                                                       dtype=np.int64)
        results = super(lg3d_DefaultFormatBundle3D, self).__call__(results)
        return results
