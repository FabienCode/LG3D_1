...
class lg3d_RandomFlip3D(RandomFlip):
    ...
    def __init__(self,
                 ...
                 flip_ratio_bev_vertical=0.0,
                 **kwargs):
        super(lg3d_RandomFlip3D, self).__init__(
            flip_ratio=flip_ratio_bev_horizontal, **kwargs)
        self.sync_2d = sync_2d
        self.flip_ratio_bev_vertical = flip_ratio_bev_vertical
        if flip_ratio_bev_horizontal is not None:
    ...
    def random_flip_data_3d(self, input_dict, direction='horizontal'):
        ...
        if 'bbox3d_fields' not in input_dict:
            ...
            if 'label_points' in input_dict:
                input_dict['label_points'].flip(direction)
            return
        if len(input_dict['bbox3d_fields']) == 0:  # test mode
            input_dict['bbox3d_fields'].append('empty_box3d')
            input_dict['empty_box3d'] = input_dict['box_type_3d'](
        ...
        for key in input_dict['bbox3d_fields']:
            if 'points' in input_dict:
                input_dict['points'] = input_dict[key].flip(
                    direction, points=input_dict['points'])
                if 'label_points' in input_dict:
                    input_dict['label_points'] = input_dict[key].flip(
                        direction, points=input_dict['label_points']
                    )
            else:
                input_dict[key].flip(direction)
        if 'centers2d' in input_dict:
    ...
    def __call__(self, input_dict):
        ...
        """
        # flip 2D image and its annotations
        super(lg3d_RandomFlip3D, self).__call__(input_dict)
...
class lg3d_GlobalAlignment(object):
    ...
    def _rot_points(self, input_dict, rot_mat):
        ...
        # input should be rot_mat_T so I transpose it here
        input_dict['points'].rotate(rot_mat.T)
        input_dict['label_points'].rotate(rot_mat.T)
    ...
    def _rot_bbox_points(self, input_dict):
        ...
        for key in input_dict['bbox3d_fields']:
            if len(input_dict[key].tensor) != 0:
                ...
                input_dict['pcd_rotation_angle'] = noise_rotation
                if 'label_points' in input_dict:
                    label_points, _ = input_dict[key].rotate(
                        noise_rotation, input_dict['label_points']
                    )
