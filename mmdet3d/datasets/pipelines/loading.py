...
class lg3d_LoadPointsFromFile(object):
    ...
    def __call__(self, results):
        ...
        points = points.reshape(-1, self.load_dim)
        points = points[:, self.use_dim]
        label_tmp_points = copy.deepcopy(points)
        attribute_dims = None
        ...
        if 'ann_info' in results:
            ...
            if self.use_color:
                ...
                if label_attribute_dims is None:
                    label_attribute_dims = dict()
                label_attribute_dims.update(
                    dict(color=[
                        points.shape[1] - 3,
                        points.shape[2] - 2,
                        points.shape[3] - 1,
            ...
            label_points_class = get_points_type(self.coord_type)
            label_points = label_points_class(
                label_points, points_dim=label_points.shape[-1], attribute_dims=label_attribute_dims
            )
